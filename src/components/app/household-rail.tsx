...
const CreateHouseholdModal = dynamic(
  () =>
    import("@/components/household/create-household-modal").then(
      (module) => module.CreateHouseholdModal,
    ),
  { ssr: false },
);
...
const AccountSettingsModal = dynamic(
  () =>
    import("@/components/settings/account-settings-modal").then(
      (module) => module.AccountSettingsModal,
    ),
  { ssr: false },
);
...
export function HouseholdRail({
  ...
  return (
    <>
      ...
      {createOpen && (
        <CreateHouseholdModal onClose={() => setCreateOpen(false)} />
      )}
